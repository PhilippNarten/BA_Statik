...
for i in range(1,n+1):
    ...
    for n_element in range(n_knoten-1):
        elemente.append(Timoshenkobalken(platzhalter, platzhalter, [sys_knoten[n_element], sys_knoten[n_element+1]], EI, EA, GA_s, element_lasten))
...
plt.plot(x_val, phi_list, color="orange", linestyle="dashdot", label="\u03C6 Timoshenko locking")
plt.plot(x_val, [phi_b]*len(x_val), color="orange", label="\u03C6 Euler-Bernoulli")
plt.plot(x_val, w_list, color="blue", linestyle='dashdot', label="w Timoshenko locking")
...
#plot speichern
datei_name = "timoshenko_kragarm_konvergenz_locking" + ".pdf"
